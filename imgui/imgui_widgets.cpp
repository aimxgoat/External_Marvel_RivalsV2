...
void ImGui::ToggleButton(const char* str_id, bool* v)
{
    ...
    */
    ...
    //    if (*v)
    //    {
    //        col_bg = IM_COL32(96, 0, 249, 255);
    //        cir_bg = IM_COL32(96, 0, 249, 255);
    //    }
    //    else
    //    {
    //        col_bg = IM_COL32(96, 0, 249, 255);
    ...
    //{
        if (*v)
        {
            col_bg = IM_COL32(0, 0, 0, 255); // 128, 128, 128, 255
            cir_bg = IM_COL32(54, 67, 108, 255);  // Dark purple circle background
        }
        else
        {
            col_bg = IM_COL32(0, 0, 0, 255);
            cir_bg = IM_COL32(255, 255, 255, 255);  // Dark purple circle background
        }
    //}

    if (*v)
    {
        draw_list->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + width, p.y + (height)), col_bg, rounding); // This is the white out lines
    }
    else
    {
    ...
    draw_list->AddCircleFilled(ImVec2(p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - ((height - radius) / 2), cir_bg);
...
void setupSnowflakes(std::vector<ImVec2>& snowflakes, ImVec2 bounds) {
    for (auto& flake : snowflakes) {
        flake.x = static_cast<float>(rand() % static_cast<int>(bounds.x));
        flake.y = static_cast<float>(rand() % static_cast<int>(bounds.y));
    }
}

void ImGui::Snowfall(ImDrawList* d, ImVec2 b)
{
    static std::vector<ImVec2> snowflakes(N);
    static auto once = (setupSnowflakes(snowflakes, b), true);
    for (auto& flake : snowflakes) {
        flake.y += 1.0f; // Adjust the speed of snowfall here
        if (flake.y > b.y) { // If snowflake reaches bottom, reset its position to the top
            flake.x = static_cast<float>(rand() % static_cast<int>(b.x));
            flake.y = 0;
        }
    }
    for (const auto& flake : snowflakes) {
        d->AddCircleFilled(flake, 2.0f, ImColor(255, 255, 255)); // Draw snowflakes as filled circles
    }
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        if (focus_requested || clicked || g.NavActivateId == id || g.NavInputSource == id)
        {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (focus_requested || (clicked && g.IO.KeyCtrl) || g.NavInputSource == id)
            {
    ...
    ImColor bodyColor = ImColor(int(0), int(140), int(255), 255);
    // Render grab
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), ImColor(46, 46, 46, 255));

    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 8, ImColor(46, 46, 46, 255));
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, (ImColor(46, 46, 46, 255)));
    }
